...
namespace RNCL_COAL
{
    ...
    class ColorMap
    {
       ...
       public static ColorMap_Type colorMap_Type = ColorMap_Type.WhiteHot;

		public static void ColorBar(byte[] bArr,  out byte[] rgbValues,int max_i, int max_j, int min_i, int min_j)
        {
			
			byte[,] ColorMap = new byte[256, 3];
		
			if (colorMap_Type == ColorMap_Type.Rainbow) ColorMap = RainbowMap;
            else if (colorMap_Type == ColorMap_Type.BlueRed) ColorMap = BlueRedMap;
			else if (colorMap_Type == ColorMap_Type.Medical)ColorMap = MedicalMap;
			else if (colorMap_Type == ColorMap_Type.Purple)ColorMap = PurpleMap;
			else if (colorMap_Type == ColorMap_Type.PurpleYellow)ColorMap = PurpleYellowMap;
			else if (colorMap_Type == ColorMap_Type.DarkBlue)	ColorMap = DarkBlueMap;
			else if (colorMap_Type == ColorMap_Type.Cyan)ColorMap = CyanMap;
			else if (colorMap_Type == ColorMap_Type.Iron) ColorMap = IronMap;

			#region 실패코드
			//bArr 데이터 조사 여기서 해서 최소 최대 뽑아도 될듯. //X  안됨.
			//============================

			//byte max_temp = bArr[0];
			////  int max_i = 0,max_j=0;
			//byte min_temp = bArr[0];
			//// int min_i = 0, min_j = 0;
			...
			//    {
			//        temp = bArr[numBytes_];
			//        numBytes_ = (i * (288 * 1)) + (j * 1);
			//        if (temp == 0 || temp == 255) continue; //bad pixel? or noise?
			...
			//int low_intensitiy = max_temp;
			//int high_intensitiy = min_temp;
			#endregion 실패코드

			int low_intensitiy = bArr[(min_j * (384 * 1)) + (min_i * 1)];
            int high_intensitiy = bArr[(max_j * (384 * 1)) + (max_i * 1)];
            int sub = (low_intensitiy - high_intensitiy);
            double delta = sub / (double)288;
            double tmp = high_intensitiy;

            int numBytes = 0;



			if (colorMap_Type == ColorMap_Type.WhiteHot)
			{
				rgbValues = new byte[288 * 24];
			...
			else if (colorMap_Type == ColorMap_Type.BlackHot)
            {
                rgbValues = new byte[288 * 24];
                ...
                for (int j = 0; j < 288; j++)
                {
                    ...
                    {
                        numBytes = (j * (24 * 1)) + (i * 1);
                        rgbValues[numBytes] = (byte)(255 - tmp);
            ...
            else if (colorMap_Type == ColorMap_Type.BlueRed || colorMap_Type == ColorMap_Type.Iron)
            {
                rgbValues = new byte[288 * 24 * 3];
                ...
                for (int j = 0; j < 288; j++)
                {
                    ...
                    {
                        numBytes = (j * (24 * 1)) + (i * 1);
                        rgbValues[numBytes * 3] = ColorMap[255 - (int)tmp, 2];
                        rgbValues[numBytes * 3 + 1] = ColorMap[255 - (int)tmp, 1];
                        rgbValues[numBytes * 3 + 2] = ColorMap[255 - (int)tmp, 0];
            ...
            else
            {
                ...
                for (int j = 0; j < 288; j++)
                {
                    ...
                    {
                        numBytes = (j * (24 * 1)) + (i * 1);
                        rgbValues[numBytes * 3] = ColorMap[(int)tmp, 2];
                        rgbValues[numBytes * 3 + 1] = ColorMap[(int)tmp, 1];
                        rgbValues[numBytes * 3 + 2] = ColorMap[(int)tmp, 0];
        ...
        public static void Change_ColorMap_BlackHot(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes] = (byte)(255 - bArr[numBytes]);
		...
		public static void Change_ColorMap_IRon(byte[] bArr, out byte[] rgbValues)
        {
            ...
            for (int i = 0; i < 384; i++)
            {
                for (int j = 0; j < 288; j++)
                {
                    ...
                    rgbValues[numBytes * 3] = IronMap[255-bArr[numBytes], 2];
                    rgbValues[numBytes * 3 + 1] = IronMap[255-bArr[numBytes], 1];
                    rgbValues[numBytes * 3 + 2] = IronMap[255-bArr[numBytes], 0];

                }
            }
        }

        public static void Change_ColorMap_Medical(byte[] bArr, out byte[] rgbValues)
        {
            rgbValues = new byte[288 * 384 * 3];
            ...
            for (int i = 0; i < 384; i++)
            {
                for (int j = 0; j < 288; j++)
                {
                    ...
                    rgbValues[numBytes * 3] = MedicalMap[bArr[numBytes], 2];
                    rgbValues[numBytes * 3 + 1] = MedicalMap[bArr[numBytes], 1];
                    rgbValues[numBytes * 3 + 2] = MedicalMap[bArr[numBytes], 0];

                }
            }
        }
		public static void Change_ColorMap_BlueRed(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = BlueRedMap[255-bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = BlueRedMap[255-bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = BlueRedMap[255-bArr[numBytes], 0];

				}
			}
		}

		public static void Change_ColorMap_Rainbow(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = RainbowMap[bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = RainbowMap[bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = RainbowMap[bArr[numBytes], 0];
		...
		public static void Change_ColorMap_Purple(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = PurpleMap[bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = PurpleMap[bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = PurpleMap[bArr[numBytes], 0];

				}
			}
		}
		public static void Change_ColorMap_PurpleYellow(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = PurpleYellowMap[bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = PurpleYellowMap[bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = PurpleYellowMap[bArr[numBytes], 0];

				}
			}
		}
		public static void Change_ColorMap_DarkBlue(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = DarkBlueMap[bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = DarkBlueMap[bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = DarkBlueMap[bArr[numBytes], 0];

				}
			}
		}
		public static void Change_ColorMap_Cyan(byte[] bArr, out byte[] rgbValues)
		{
			rgbValues = new byte[288 * 384 * 3];
			...
			for (int i = 0; i < 384; i++)
			{
				for (int j = 0; j < 288; j++)
				{
					...
					rgbValues[numBytes * 3] = CyanMap[bArr[numBytes], 2];
					rgbValues[numBytes * 3 + 1] = CyanMap[bArr[numBytes], 1];
					rgbValues[numBytes * 3 + 2] = CyanMap[bArr[numBytes], 0];

				}
			}
		}

        static byte[,] IronMap = new byte[256, 3]{
            { 255, 255, 250 },{ 255, 255, 247 },{ 255, 255, 242 },{ 255, 255, 235 },{ 255, 254, 229 },{ 255, 254, 223 },{ 255, 253, 218 },
    ...
    { 0, 0, 71 },{ 0, 0, 66 },{ 0, 0, 61 },{ 0, 0, 56 },{ 1, 0, 51 },{ 1, 1, 46 },{ 0, 0, 41 },{ 0, 0, 36 } };
        ...
        static byte[,] MedicalMap = new byte[256, 3]{ { 0, 0, 0 },{ 8, 0, 8 },{ 16, 0, 16 },{ 24, 0, 24 },{ 32, 0, 32 },{ 40, 0, 40 },{ 48, 0, 48 },{ 56, 0, 56 },{ 65, 0, 65 },
    ...
    { 168, 50, 50 },{ 159, 50, 50 },{ 149, 50, 50 },{ 139, 50, 50 },{ 130, 50, 50 },{ 120, 50, 50 },{ 110, 50, 50 } };
        ...
        static byte[,] BlueRedMap = new byte[256, 3]{
		...
		static byte[,] RainbowMap = new byte[256,3]{
		...
		static byte[,] CyanMap = new byte[256,3]{
		...
		static byte[,] DarkBlueMap=new byte[256,3]{
		...
		static byte [,] PurpleYellowMap=new byte[256,3] {
		...
		static byte[,] PurpleMap = new byte[256, 3] {
